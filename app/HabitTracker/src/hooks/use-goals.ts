...
export function useGoals() {
    ...
    try {
      ...
      setError(
        loadError instanceof Error
          ? loadError
          : new Error('No se pudieron cargar los objetivos'),
      );
    } finally {
      setLoading(false);
    }
  ...
  const updateGoal = useCallback(
    (habitType: HabitType, changes: GoalChanges) => {
      setGoals((current) => {
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      await Promise.all(
        Object.values(goals).map((goal) => saveGoal(goal)),
      );
      ...
      const nextError =
        saveError instanceof Error
          ? saveError
          : new Error('No se pudieron guardar los objetivos');
